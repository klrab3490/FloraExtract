    ...
    <div>
      <div className="w-[100vw] flex justify-center items-center px-2 py-2 font-semibold">
        <div className="px-4 py-2 hidden sm:flex">
          <NavLink
            to={"/shop"}
            className="text-2xl text-[#290c06] hover:text-[#fd9b40] transition duration-300 cursor-pointer"
          >
            Products
          </NavLink>
        </div>
      ...
      <div
        className={`fixed left-0 w-full top-28 z-50 ${isOpen ? "" : "hidden"}`}
      >
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                exit="closed"
                variants={variants}
                className="xl:hidden border-[1px] border-[#290c06] w-1/2 p-5 space-y-4 rounded-xl text-2xl flex-col items-center justify-center bg-[#fdfde1] text-center z-30"
                
              >
                <motion.div
                  onClick={handleClose}
                  variants={itemVariants}
                  className="border-[1px] border-[#290c06] py-2 rounded-xl"
